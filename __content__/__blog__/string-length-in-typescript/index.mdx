...
Sadly it doesn't work the way we wanted. We got `number` type. Technically, there's nothing wrong with that – the type of string length *is* indeed a  *number*. But what about the *exact* number – *11* in case of "marshmallow"?

Sidebar: what about arrays? Can TypeScript get exact length of an array? Let's try:
...
Still no success. Well, TypeScript actually can't get the exact length of an array. The only data structure it *can* get length of is a **tuple**, a list-like data structure with a fixed length. JavaScript currently doesn't have a tuple data structure, but TypeScript kind of does – a readonly array.
...
To get to the point: we *can* get the exact length of a string, if we convert it somehow to a tuple of the same length. Is this possible? Good news – it is!

This might get a little scary for someone, but there is nothing too arcane, just a simple tail-call recursion, which is a substitute for a loop (TypeScript is currently very limited regarding loops – only the `in` syntax can be probably considered a loop).
